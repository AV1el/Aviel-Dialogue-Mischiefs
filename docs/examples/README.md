...
A ready-to-copy content set: a guard NPC with a small quest (flags, item turn-in, rewards), a trade shop gated behind quest completion, and an entity mapping.
...
- The dialogue plays `<anim:wave>` on quest completion — export a `wave.json` emote (EmoteCraft format) into `config/adm-dialogues/emotes/`, or remove the tag. A missing emote is skipped and reported by `/adm_npc validate`.
